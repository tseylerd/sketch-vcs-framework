...
+ (void) exec: (NSArray*) args client: (NSURL *)url_client document: (MSDocument *) document {
    ...
    [GitPlugin showAlert:launchPath];
...
+ (void) runCommand: (MSDocument*) document plugin: (NSObject*) plugin command: (NSString *) command {
    @try {
        ...
        if (![[documentUrl scheme] isEqualToString:@"file"]) {
            [GitPlugin showAlert:[NSString stringWithFormat:@"%@%@", @"Only local files can be under version control.", documentUrl.absoluteString]];
            return;
        }
        
        NSString* documentPath = [[documentUrl path] stringByRemovingPercentEncoding];
        [GitPlugin showAlert:documentPath];
        ...
        if ([command isEqualToString:@"start_id"]) {
            [GitPlugin exec: [NSArray arrayWithObjects:@"start", documentPath, nil] client:url_client document: document];
        }
        else if ([command isEqualToString:@"stop_id"]) {
            [GitPlugin exec: [NSArray arrayWithObjects:@"stop", documentPath, nil] client:url_client document: document];
        }
        else if ([command isEqualToString:@"pull_id"]) {
            [GitPlugin exec: [NSArray arrayWithObjects:@"pull", documentPath, nil] client:url_client document: document];
        } else if ([command isEqualToString:@"push_id"]) {
            NSString* commitMessage = [GitPlugin getUserInput:@"Enter commit message:"];
            if (commitMessage == nil) {
                return;
            }
            [GitPlugin exec: [NSArray arrayWithObjects:@"push", documentPath, commitMessage, nil] client:url_client document: document];
        }
    }
    @catch (NSException *e) {
        NSLog(@"%@", [e reason]);
